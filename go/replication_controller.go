...
func k8s_client_corev1_rcs_list(clientsetKey C.uintptr_t, namespace *C.char, optsBytes unsafe.Pointer, optsSize C.int, oBytes *unsafe.Pointer, oSize *C.int) *C.char {
	listOptions := metav1.ListOptions{}
	listOptions.Unmarshal(no_copy_slice_from_c_array(optsBytes, optsSize))
	objList, err := clientsetMap[clientsetKey].CoreV1().ReplicationControllers(C.GoString(namespace)).List(listOptions)
	if err != nil {
...
func k8s_client_corev1_rcs_watch(clientsetKey C.uintptr_t, namespace *C.char, optsBytes unsafe.Pointer, optsSize C.int,
callbackFn C.uintptr_t, callbackContext C.uintptr_t) *C.char {
	listOptions := metav1.ListOptions{}
	listOptions.Unmarshal(no_copy_slice_from_c_array(optsBytes, optsSize))
	listOptions.ResourceVersion = "0"
	watch, err := clientsetMap[clientsetKey].CoreV1().ReplicationControllers(C.GoString(namespace)).Watch(listOptions);
	if err != nil {
		return C.CString(err.Error())
	}
